...
export PROJECT_NAME=teste_backend
...
function db_setup {
  docker exec -ti ${PROJECT_NAME}-db psql -U postgres -c "create database teste_backend_development"
  docker exec -ti ${PROJECT_NAME}-db psql -U postgres -c "create database teste_backend_test"
  dk "yarn run:migrations:dev"
  dk "yarn run:migrations:test"
  dk "yarn run:seeds"
}
